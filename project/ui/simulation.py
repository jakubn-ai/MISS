...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, idx):
    ...
    def kill(self) -> None:
        vehicles[self.direction][self.lane].pop(self.index)
        for v in vehicles[self.direction][self.lane]:
            v.index -= 1
...
def generateVehicles():
    ...
    while (len(ACTUAL_CARS)) <= 5:
        ...
        if (temp < dist[0]):
            ...
            direction_number = 3
        car_value += 1
        Vehicle(lane_number, vehicleTypes[vehicle_type], 0, directionNumbers[0],
                    car_value)
        time.sleep(1)
...
class Main:
    ...
    while True:
        ...
        for vehicle in simulation:
            ...
            ACTUAL_CARS[cars] = vehicle
            if vehicle.to_delete:
                ACTUAL_CARS.pop(vehicle.idx)
                vehicle.kill()
            vehicle.move()
